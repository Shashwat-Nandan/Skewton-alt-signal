...
- **Options backtest / chain loaders**:
  `.venv/bin/python backtest.py --data data_cache/NIFTY_<from>_<to>.parquet`
...
- **Pair screener / raw bhavcopy loaders** (same loader the LIVE pair runner
  uses): `screen_pairs.py --output <scratch>/pair_candidates_test.csv` —
  ...
  latest session.
- **Kalman trend loaders**: `backtest_kalman_trend.py --csv
  data_cache/NIFTY_5minute.parquet` (CMA-ES fit is slow — minutes).

## Do NOT drive live

- Any `run_paper*.py` / `run.py` (paper/live runners, need Kite session).
