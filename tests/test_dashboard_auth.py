...
class TestSessionEndpoints:
    def test_me_returns_false_when_no_cookie(self, unauthed_client):
        r = unauthed_client.get("/session/me")
        assert r.status_code == 200
        assert r.json() == {"authenticated": False}

    def test_login_with_correct_password_sets_cookie(self, unauthed_client):
        r = unauthed_client.post("/session/login", json={"password": "test-password"})
        ...
        r2 = unauthed_client.get("/session/me")
        assert r2.json() == {"authenticated": True}

    def test_login_with_wrong_password_returns_401(self, unauthed_client):
        r = unauthed_client.post("/session/login", json={"password": "wrong"})
        assert r.status_code == 401
        # Still no session.
        assert unauthed_client.get("/session/me").json() == {"authenticated": False}

    def test_login_with_empty_password_rejected_by_validation(self, unauthed_client):
        r = unauthed_client.post("/session/login", json={"password": ""})
        assert r.status_code == 422  # Pydantic min_length=1

    def test_logout_clears_session(self, unauthed_client):
        unauthed_client.post("/session/login", json={"password": "test-password"})
        assert unauthed_client.get("/session/me").json() == {"authenticated": True}
        r = unauthed_client.post("/session/logout")
        assert r.status_code == 204
        assert unauthed_client.get("/session/me").json() == {"authenticated": False}
...
class TestGating:
    @pytest.mark.parametrize(
        ...
        [
            ("GET", "/strategies"),
            ("GET", "/strategies/pair_trading/params"),
            ("GET", "/runs"),
            ("POST", "/runs"),
            ("GET", "/runs/some-id"),
            ("POST", "/runs/some-id/stop"),
            ("GET", "/auth/status"),
            ("GET", "/auth/login"),
            ("POST", "/auth/logout"),
            ("GET", "/market-profile/symbols"),
            ("GET", "/market-profile/RELIANCE"),
            ("GET", "/pair-candidates"),
        ],
    )
    ...
    def test_tampered_cookie_rejected(self, unauthed_client):
        ...
        r = unauthed_client.get("/strategies")
        assert r.status_code == 401
