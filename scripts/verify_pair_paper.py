...
Inputs (read-only):
  - data_cache/pair_paper_eod_<today>.json     written by runners/run_paper_pairs.py
...
# HOLIDAYS_PATH comes from the same place for the same reason.
from runners.run_paper_pairs import (  # noqa: E402
    HOLIDAYS_PATH,
    is_trading_day,
    load_holidays,
)
...
def main():
    ...
    parser.add_argument("--force", action="store_true",
                        help="Run the verifier even on weekends / NSE holidays. "
                             "Mirrors --force in runners/run_paper_pairs.py — only set "
    ...
    if sidecar is None:
        ...
        log.error("No EOD sidecar at data_cache/%s%s.json — has "
                  "runners/run_paper_pairs.py (system=%s) run today?",
