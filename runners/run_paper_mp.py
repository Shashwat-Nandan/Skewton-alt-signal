...
dashboard.db (kept current by `python -m market_data.fetch_bars --update`) and books fills at the
...
from core.market_profile import Bar, DayProfile, compute_day_profile, split_by_day
...
def main() -> None:
    ...
    if not all_dates:
        logger.warning("No %dm bars in dashboard.db — run market_data/fetch_bars.py first.", args.interval)
        return
