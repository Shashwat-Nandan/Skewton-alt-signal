...
`fetch_bars.py` (271 lines) is the daily 30-min bar ingester. Unlike
`fetch_bhavcopy.py` which gives EOD-only schema from NSE archives,
...
of truth — see `fetch_bars.py:49–53`).
...
- Token must be valid: `KiteAuthManager` from `kite_auth.py`
...
| `analyze_rv_iv_regime.py` | Intra-day RV calibration |
...
| File | Role |
|---|---|
| `fetch_bars.py` | Daily 30-min bar ingester |
| `scripts/replay_missed_bars.py` | Backfill ops tool |
| `fetch_historical_data.py` | Older Kite-API-based daily/intraday fetcher (mostly superseded) |
